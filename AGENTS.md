...
1. **맥락 우선**: 작업 전 반드시 관련 skills/, pipelines/, sop/ 문서 확인
...
```
jw-agent-workspace/
├── AGENTS.md              ← 지금 이 파일 (전역 컨텍스트, 항상 먼저 읽기)
├── memory/
│   ├── clients.md         ← 클라이언트 현황 및 주요 메모
│   ├── ongoing.md         ← 진행중 프로젝트 트래킹
│   └── daily-log/         ← 날짜별 업무일지 (YYYY-MM-DD.md)
├── skills/
│   ├── content-creation.md   ← 콘텐츠 작성 스킬
│   ├── client-report.md      ← 고객 보고서 생성 스킬
│   └── kakao-message.md      ← 카카오 메시지 초안 스킬
├── pipelines/
│   ├── morning-briefing.md   ← 모닝 브리핑 파이프라인
│   └── consult-analysis.md   ← 상담 분석 파이프라인
├── sop/
│   ├── new-client-onboarding.md  ← 신규 고객 온보딩 SOP
│   └── weekly-content.md         ← 주간 콘텐츠 발행 SOP
└── rules/
...
_관리자: 남진우 / jw-agent-workspace v1.0_
